var iFileName = "Cleric - Time Domain (Revised) [Jonoman3000] (modified).js";
...
AddSubClass("cleric", "time domain", { // Longstrider replaced with Identify per request
	regExpSearch : /^(?=.*(cleric|priest|clergy|acolyte))(?=.*\b(time|temporal|chronal)\b).*$/i,
	subname : "Time Domain",
	source : ["HB"],
	spellcastingExtra : ["identify", "sleep", "blur", "gentle repose", "haste", "slow", "divination", "dimension door", "modify memory", "transmute rock"],
	features : {
		"subclassfeature1" : {
			name : "Bonus Cantrip",
			source : ["HB"],
